...
function Card({ issue }: { issue: Issue }) {
    ...
    const openDialog = () => {
        // TODO: Update this to open the dialog
        console.log(issue);
    }
    ...
    return (
        repo?.language && <div className={styles.card} onClick={openDialog}>
            ...
            <div className={styles.card__labels}>
                <p className={styles.label}>{repo?.language}</p>
                {
                    issue.labels.map((label, id) => {
                        ...
                        return (
                                label.name != "good first issue" && <p key={id} className={styles.label} title={label.description} style={color}>{label.name}</p>
                            )
                    })
                }
            </div>
